...
def konys():
    ...
    glColor4f(0.6471, 0.5412, 0.9843, 0.4)  # Цвет + прозрачность
...
def cylinder():
    ...
    glColor3f(1, 1, 1)  # Белый цвет для каркаса
    glBegin(GL_LINES)
    for i in range(num_segments):
        angle = 2 * math.pi * i / num_segments
        x = radius * math.cos(angle)
...
def sphere():
    ...
    glColor4f(0.6471, 0.5412, 0.9843, 0.5)  # Цвет + прозрачность
    glBegin(GL_QUADS)
...
def run_pygame(draw_function, persp):
    ...
    gluPerspective(persp, (display[0] / display[1]), 0.1, 50.0)
