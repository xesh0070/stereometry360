...
def triangular_prism():
    ...
    edges = (
        ...
        (0, 3,), (1, 4,), (2, 5,),  # Боковые ребра
        (7, 8,), (8, 9,), (9, 6,)  # Дополнительные ребра
    )
    ...
    faces = [
        (0, 1, 2),  # Нижняя грань
        (3, 4, 5),  # Верхняя грань
        (0, 3, 4, 1),  # Боковая грань 1
        (1, 4, 5, 2),  # Боковая грань 2
        (2, 5, 3, 0)  # Боковая грань 3
    ]
    ...
    glBegin(GL_TRIANGLES)
    for face in faces2:
        glColor4f(0.5255, 0.0275, 0.9608, 0.7)
        for vertex in face:
            glVertex3fv(vertices[vertex])
    glEnd()
...
def run_pygame_tip14_ygol1_sv(e, stop_event_local):
...
def start_pygame(e):
    threading.Thread(target=run_pygame_tip14_ygol1_sv(e)).start()
